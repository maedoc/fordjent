...
### Requirement: RalphGuard enforces spec immutability
During ralph sessions, the harness SHALL prevent modification of spec files under `openspec/`. Two layers of enforcement are used: `write_file` rejects paths under `openspec/` during ralph, and the `git` tool's commit handler inspects the staged diff and rejects any commit touching `openspec/**/spec.md`.

#### Scenario: write_file blocked on spec path
- **GIVEN** a ralph session is active
- **WHEN** the agent calls `write_file` with path `openspec/changes/foo/spec.md`
- **THEN** the call fails with error: "Error: spec files are immutable during ralph"

#### Scenario: Commit gate blocks spec changes via bash
- **GIVEN** a ralph session where the agent used bash to modify `openspec/changes/foo/spec.md`
- **WHEN** the agent attempts a `git commit`
- **AND** the staged diff includes `openspec/changes/foo/spec.md`
- **THEN** the commit is rejected with error: "Error: commits may not modify spec files during ralph"
