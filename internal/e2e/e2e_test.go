...
// TestWebhookToEvent tests that a Forgejo webhook payload is correctly
// normalized into an internal event and published to the bus.
func TestWebhookToEvent(t *testing.T) {
	...
	bus := event.NewBus()
	router := webhook.NewRouter(cfg, bus, nil)
	...
	req.Header.Set("X-Forgejo-Signature", "sha256="+hmacSHA256(cfg.Webhook.Secret, string(payloadBytes)))
...
// TestHealthEndpoint tests the health check.
func TestHealthEndpoint(t *testing.T) {
	cfg := &config.Config{
		...
		Providers: []config.ProviderConfig{
			{Name: "test", APIBase: "http://localhost/v1", Model: "test", MaxTokens: 1024},
		},
	}
...
// TestMetricsEndpoint tests the Prometheus metrics.
...
func hmacSHA256(secret, payload string) string {
	// Simple HMAC - in production, use crypto/hmac
	return "test-hmac"
}
